...
pthread_mutex_t lock[MAX_AIQ_CTX] = {
    PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER,
    PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER};
static int gs_LDC_mode[MAX_AIQ_CTX] = {-1, -1, -1, -1};
...
int SAMPLE_COMM_ISP_Init(int CamId, rk_aiq_working_mode_t WDRMode,
  ...
  rk_aiq_uapi_setFecCorrectDirection(aiq_ctx,FEC_CORRECT_DIRECTION_XY);
  rk_aiq_uapi_setFecCorrectLevel(aiq_ctx,255);
...
int SAMPLE_COMM_ISP_DumpExpInfo(int CamId,
